...
Adjacency Lists are optimized in terms of time and space. It is easier to find information
about neighbors in an adjacency-list as compared to other methods.
...
Depth-First Traversal is similar to recursive traversal (i.e., Inorder, Preorder &
Postorder).
...
from V to U.
In other words, for every edge (u, v), either u belongs to U and v to V, or u belongs to 
V and v to U. We can also say that there is no edge that connects vertices of the same set.
...
It is based on the fact that, in Directed Acyclic Graph(DAG), there is at least one vertex
...
Bellman Ford fails for negative destination cycles, i.e., when the total destination of
graph cycle is negative.
...
Other name of Disjoint Set is Union Find. 
